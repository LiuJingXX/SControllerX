...
//        }
    ...
    private void drawTag(Canvas canvas, float x, float y, float closeToScreenEdgeFactor, TagNode tag, int alpha) {
        ...
        if (tagLabelAlpha > 0) {
            ...
            nodeSubInfoPaint.setAlpha(alpha);
            //noinspection ConstantConditions
            String x_pos = Util.formatLength(position.x, lengthUnit);
            String y_pos = Util.formatLength(position.y, lengthUnit);
            String z_pos = Util.formatLength(position.z, lengthUnit);
            map.put("x_location",Double.valueOf(x_pos));
            map.put("y_location",Double.valueOf(y_pos));
            map.put("z_location",Double.valueOf(z_pos));

            System.out.println("--------------------------------------------------------------------------------------------------------------------");
            String strPosition = x_pos
                    + ", " + y_pos
                    + ", " + z_pos;
